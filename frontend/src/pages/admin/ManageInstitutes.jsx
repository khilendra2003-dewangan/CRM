...
import { Plus, Edit2, Trash2, Eye, Globe, Phone, Mail, MapPin, Building2 } from "lucide-react";
...
export default function ManageInstitutes() {
  ...
  const initialFormState = {
    ...
    zip: "", country: "", phone: "", email: "", website: "",
    logo: "", status: "Active"
  };
  ...
  const fetchInstitutes = async () => {
    setIsLoading(true);
    try {
      const res = await axiosInstance.get("/getAllInstitutes");
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await axiosInstance.put(`/updateInstitute/${editingId}`, formData);
      } else {
        await axiosInstance.post("/createInstitute", formData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axiosInstance.delete(`/deleteInstitute/${id}`);
      fetchInstitutes();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-[#f8fafc]">
        <div>
          <h2 className="text-xl font-semibold text-[#0f172a] font-serif">Institutes</h2>
          <p className="text-sm text-slate-500 mt-1">Manage overarching educational institutes.</p>
        </div>
        <button
          onClick={() => openForm()}
          className="bg-[#1e3a8a] hex-shadow hover:bg-[#172554] text-white px-4 py-2 rounded-xl text-sm font-medium flex items-center space-x-2 transition-colors shadow-sm"
        >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-slate-600">
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan="4" className="text-center py-8 text-slate-400">Loading institutions...</td></tr>
            ) : institutes.length === 0 ? (
              <tr><td colSpan="4" className="text-center py-8 text-slate-400">No institutes found.</td></tr>
            ) : (
              institutes.map((inst) => (
                <tr key={inst._id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4 font-semibold text-[#1e3a8a]">{inst.code}</td>
                  <td className="px-6 py-4 font-medium text-[#0f172a]">{inst.name}</td>
                  <td className="px-6 py-4 text-slate-500">
                    {inst.city ? `${inst.city}, ${inst.state}` : "Not specified"}
                  </td>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex items-start space-x-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="w-16 h-16 bg-[#1e3a8a]/10 rounded-xl flex items-center justify-center text-[#1e3a8a] border border-[#1e3a8a]/20">
                {selectedInst.logo ? (
                  <img src={selectedInst.logo} alt="Logo" className="w-full h-full object-cover rounded-xl" />
                ) : (
                  <Building2 className="w-8 h-8" />
                )}
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-bold text-[#0f172a] leading-tight mb-1">{selectedInst.name}</h3>
                ...
                <span className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedInst.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <DetailItem icon={Mail} label="Email Address" value={selectedInst.email} />
                <DetailItem icon={Phone} label="Phone Number" value={selectedInst.phone} />
                <DetailItem icon={Globe} label="Website" value={selectedInst.website} isLink />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-white border border-slate-100 rounded-2xl space-y-3">
                <div className="flex items-start space-x-3 text-sm text-slate-600">
                  ...
                  <div>
                    <p className="font-medium text-[#0f172a]">{selectedInst.address}</p>
                    <p>{selectedInst.city}, {selectedInst.state} {selectedInst.zip}</p>
                    <p className="text-xs text-slate-400 uppercase mt-1 font-bold tracking-tighter">{selectedInst.country}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="pt-6 flex space-x-3">
              <button
                onClick={() => {
                  ...
                  openForm(selectedInst);
                }}
                className="flex-1 bg-white border border-slate-200 text-[#0f172a] py-3 rounded-xl font-medium shadow-sm hover:bg-slate-50 transition-colors flex items-center justify-center space-x-2"
              >
                <Edit2 className="w-4 h-4" />
        ...
        <form onSubmit={handleSave} className="space-y-5 pb-10">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <InputItem label="Institute Code" name="code" value={formData.code} onChange={handleChange} required placeholder="e.g. TST01" />
              <InputItem label="Status" name="status" type="select" options={["Active", "Inactive"]} value={formData.status} onChange={handleChange} />
            </div>
            <InputItem label="Institute Name" name="name" value={formData.name} onChange={handleChange} required placeholder="Enter full name" />
          ...
          <div className="sticky bottom-0 bg-white pt-4 pb-2">
            <button type="submit" className="w-full bg-[#1e3a8a] text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-900/10 hover:bg-[#172554] active:scale-[0.98] transition-all">
              {editingId ? "Update Institute" : "Create Institute"}
            </button>
          </div>
        </form>
      </SlideOver>
